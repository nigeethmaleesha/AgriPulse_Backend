...
> Integrated runtime: port `8082`, database `agripulse_module1_dispatch`, PostgreSQL user `postgres`, password `1234`. Start the shared PostgreSQL service from the project-root `docker-compose.yml`.
...
*Note: If using local PostgreSQL without Docker, create database `agripulse_module1_dispatch` with user `postgres` and password `postgres`.*
...
spring.datasource.url=${DB_URL:jdbc:postgresql://localhost:5432/agripulse_module1_dispatch}
...
```text
http://localhost:8082/api/v1/dispatch
```
...
1. Seed initial data:
   ```bash
   curl -X POST http://localhost:8082/api/v1/dispatch/seed-data
   ```
2. Request dispatch route for truck at `C1`:
   ```bash
   curl -X GET "http://localhost:8082/api/v1/dispatch/next-route?truckNode=C1"
   ```
...
1. Close the road connecting `C3` to `C4` (Road ID `3`):
   ```bash
   curl -X PUT http://localhost:8082/api/v1/dispatch/roads/3/status \
...
1. Mark batch `B-102` as collected:
   ```bash
   curl -X PUT "http://localhost:8082/api/v1/dispatch/batches/B-102/collect?truckNode=C1"
   ```
