package com.agripulse.backend.dto;

import jakarta.validation.constraints.Max;
import jakarta.validation.constraints.Min;

/**
 * Shared Module 3 synthetic-network benchmark request.
 * topN is ignored by the Member 5 max-flow-only benchmark and used by
 * the Member 6 bottleneck benchmark.
 */
public record BenchmarkRequest(
        @Min(2) @Max(1000) int nodeCount,
        @Min(1) @Max(10000) int edgeCount,
        Long seed,
        @Min(1) Long minCapacityKgPerDay,
        @Min(2) Long maxCapacityKgPerDay,
        Boolean saveResult,
        @Min(1) @Max(100) Integer topN
) {
}
